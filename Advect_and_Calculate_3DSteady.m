% Input arguments:
    ...
    % U_Interp,V_Interp,W_Interp            : Velocity field interpolants with inputs (tspan,xx,yy,zz)
    % NCores                       : Number of Cores for parpool

    % Output arguments:
    %   xt,yt,zt    : xx-component, yy-component of trajectory final position
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%% Calculate TRA and TSE diagnostics for steady 3D flows.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%


function     [xt,yt,zt,time_note,TSE_Bar,TSE,TRA_Bar,TRA] = Advect_and_Calculate_3DSteady(tspan,xx,yy,zz,U_Interp,V_Interp,W_Interp,NCores)
...
spmd(cpu_num)
    ...
    for s=1:numel(tspan)-1
        ...
        [UK1,VK1,WK1] = r_prime(x_spmd(1,:),y_spmd(1,:),z_spmd(1,:),U_Interp,V_Interp,W_Interp);
        
        xx = x_spmd(1,:) + 0.5 * ds * UK1;
        yy = y_spmd(1,:) + 0.5 * ds * VK1;
        zz = z_spmd(1,:) + 0.5 * ds * WK1;
        [UK2,VK2,WK2] = r_prime(xx,yy,zz,U_Interp,V_Interp,W_Interp);
        
        
        xx = x_spmd(1,:) + 0.5 * ds * UK2;
        yy = y_spmd(1,:) + 0.5 * ds * VK2;
        zz = z_spmd(1,:) + 0.5 * ds * WK2;
        [UK3,VK3,WK3] = r_prime(xx,yy,zz,U_Interp,V_Interp,W_Interp);
        
        
        xx = x_spmd(1,:) + ds * UK3;
        yy = y_spmd(1,:) + ds * VK3;
        zz = z_spmd(1,:) + ds * WK3;
        [UK4,VK4,WK4] = r_prime(xx,yy,zz,U_Interp,V_Interp,W_Interp);
...
function [uk,vk,wk] = r_prime(xx,yy,zz,U_Interp,V_Interp,W_Interp)
...
uk=U_Interp(double(xx),double(yy),double(zz));
uk=uk.*Bly;

vk=V_Interp(double(xx),double(yy),double(zz));
vk=vk.*Bly;

wk=W_Interp(double(xx),double(yy),double(zz));
wk=wk.*Bly;
